...
So even when there's just grass for the land cover, the annual erosion rate is so small that its much less than our effective resolution of 1cm per year. So we have to run the nuclide simulation for a longer span of time, about 100 years, for there to be enough erosion to even remove one cm cell.
```{r}
readRAST(edmaps[1]) %>%
  as_tibble %>%
  select(x = s1, y = s2, ED = 1) %>%
  mutate(ED = ED * 100 * 10)  # convert meters to centimeters
```
