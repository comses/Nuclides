...
################# SET UP THESE VALUES ####################
##########################################################
RunLength = 50 #number of years the simulation ran for
...
elevmaps = grass.read_command('g.list', flags='m', type='rast', pattern='*%s*elevation*' % prefix, separator=',', mapset = mapset).strip().split(',')
depthmaps = grass.read_command('g.list', flags='m', type='rast', pattern='*%s*soildepth*' % prefix, separator=',', mapset = mapset).strip().split(',')  
...
for i in range(RunLength):
    elev, delta, sdepth = grass.read_command('r.what', map = ",".join((elevmaps[i], deltamaps[i], depthmaps[i])), separator = ",", coordinates = coor).strip('\n').split(",")[3:9]
    l.append({'Year': i, 'Elevation': float(elev), 'Soildepth': float(sdepth), 'Delta': float(delta)})
