...
charcoalmaps = []
averagecharc = []
for n, lcovmap, firemap in zip(range(len(lcovmaps)),lcovmaps, firemaps): #farmingmap, grazingmap, farmingmaps, grazingmaps,
    ...
    grass.mapcalc("${charcoal}=eval(biomass=graph(${lcov}, 0,0, 7,0.1, 18.5,0.66, 35,0.74, 50,1.95), pcntcharc=graph(${lcov}, 0,0, 5,0.0048, 18.5,0.0101, 50,0.0325), ((biomass * pcntcharc * 0.0267 * 0.5) / (0.00011304 * 1000)))", quiet=True, overwrite=True, lcov=lc, charcoal=charcoalmap, firemap=firemap)
...
for idx, row in layers.iterrows():
    if row['Delta'] > 0: # Deposition occured, so accumulate proxies and depth
        ...
        for depth in range(numdepths): # now add the correct proportion of proxy to each interval
            proxylist.append([row['InSitu Grass Phytoliths']/numdepths, row['InSitu Wood Phytoliths']/numdepths, row['Scaled Basin-Average Grass Phytoliths']/numdepths, row['Scaled Basin-Average Wood Phytoliths']/numdepths, row['InSitu Charcoal']/numdepths, row['Scaled Basin-Average Charcoal']/numdepths])
...
labels = ["Grass Phyt, insitu", "Wood Phyt, insitu", "Grass Phyt, bas. av.", "Wood Pht, bas. av.", "Macrocharcoal, insitu", "Macrocharcoal, bas. av.", "Depth"] # create some column labels (will also be used in the plot)
